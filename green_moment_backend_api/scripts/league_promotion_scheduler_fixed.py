...
class LeaguePromotionService:
    ...
    async def calculate_monthly_carbon_savings(self, db: AsyncSession, user_id: int, month: int, year: int):
        ...
        for chore in chores:
            ...
            worst_case_intensity = self._find_worst_continuous_period(
                carbon_data, chore.start_time, chore.duration_minutes
            )
    ...
    def _calculate_period_carbon_intensity(self, carbon_data, start_time, end_time):
        ...
        # Ensure start_time and end_time are timezone-naive for comparison
        ...
        while current_time <= end_time:
            ...
            if current_time in carbon_data:
                ...
                for ts, intensity in carbon_data.items():
                    # Make sure we're comparing timezone-naive datetimes
                    ts_naive = ts if ts.tzinfo is None else ts.replace(tzinfo=None)
                    diff = abs(ts_naive - current_time)
                    if min_diff is None or diff < min_diff:
                        min_diff = diff
                        closest_intensity = intensity
                
                if min_diff is not None and min_diff < timedelta(hours=1):
    ...
    def _find_worst_continuous_period(self, carbon_data, start_time, duration_minutes):
        """Find the worst continuous period of the day for the given duration"""
        # Get the date from start_time, handling timezone-aware datetime
        if start_time.tzinfo is not None:
        ...
        for ts, intensity in carbon_data.items():
            ...
            ts_date = ts.date() if ts.tzinfo is None else ts.replace(tzinfo=None).date()
            if ts_date == date:
    ...
    async def check_and_promote_user(self, db: AsyncSession, user: User):
        ...
        # No notifications - removed notification code
