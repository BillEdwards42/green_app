#!/usr/bin/env python3
"""
Fixed script to test push notifications with your phone
"""
...
async def test_with_real_token(user_email: str, fcm_token: str):
    ...
    async with AsyncSessionLocal() as db:
        ...
        # Convert user.id to string for device token
        user_id_str = str(user.id)
        
        # Update or create device token
        device_token = await db.execute(
            select(DeviceToken).where(
                DeviceToken.user_id == user_id_str,
        ...
        if device_token:
            ...
            device_token = DeviceToken(
                user_id=user_id_str,  # Already converted to string
                token=fcm_token,
                platform="android",
        ...
        if result['success']:
            ...
            print("\n💡 Next steps:")
            print("1. Check if notification appeared on your phone")
            print("2. If yes, you can now run the scheduler: python scripts/run_notification_scheduler.py")
            print("3. Set your preferred notification time in the app settings")
        else:
...
async def main():
    print("🚀 Green Moment Push Notification Test (Fixed)")
    print("=" * 50)
